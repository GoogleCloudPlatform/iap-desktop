...
namespace Google.Solutions.CloudIap.Plugin.Gui
{
    public partial class AuthorizeDialog : Form
    {
        ...
        public AuthorizeDialog()
        {
            ...
            var canUseGcloud = GcloudAuthorization.CanAuthorize(gcloudAccount);
            
            if (canUseGcloud)
            {
                var width = this.useGcloudLink.Width;
                this.useGcloudLink.Text += $" for {gcloudAccount.Name}";
            }
            this.useGcloudLink.Enabled = canUseGcloud;
        }
        ...
        private void ToggleSignInButton()
        {
            ...
            this.useGcloudLink.Visible = !this.useGcloudLink.Visible;
        }
            ...
            using (var dialog = new AuthorizeDialog())
            {
                Task.Run(async () =>
                {
                    try
                    {
                        ...
                        {
                            // Try to authorize using Gcloud.
                            dialog.authorization = await GcloudAuthorization.TryLoadExistingAuthorizationAsync(
                                GcloudAccount.ActiveAccount,
                                dataStore);
                        }
                ...
                dialog.useGcloudLink.MouseUp += async (sender, args) =>
                {
                    dialog.authorization = await GcloudAuthorization.CreateAuthorizationAsync(
                        GcloudAccount.ActiveAccount,
                        dataStore);
