...
namespace Plugin.Google.CloudIap.Gui
{
    public partial class GenerateCredentialsDialog : Form
    {
        ...
        {
            var dialog = new GenerateCredentialsDialog();
            dialog.usernameText.Text = suggestedUsername;

            if (dialog.ShowDialog(owner) == DialogResult.OK &&
                !String.IsNullOrWhiteSpace(dialog.usernameText.Text))
            {
                return dialog.usernameText.Text;
            }
            else
            {
                return null;
