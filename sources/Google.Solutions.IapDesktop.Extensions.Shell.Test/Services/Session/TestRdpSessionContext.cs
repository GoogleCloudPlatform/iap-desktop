...
namespace Google.Solutions.IapDesktop.Extensions.Shell.Test.Services.Session
{
    ...
    public class TestRdpSessionContext
    {
        ...
        {
            var parameters = new RdpSessionContext(
                new Mock<ITunnelBrokerService>().Object,
                SampleInstance,
                new RdpCredential("user", null, null),
                RdpSessionContext.ParameterSources.Inventory).Parameters;

            Assert.AreEqual(RdpSessionContext.DefaultPort, parameters.Port);
            Assert.AreEqual(RdpSessionContext.DefaultConnectionTimeout, parameters.ConnectionTimeout);
